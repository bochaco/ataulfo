...
export class AtaulfoSimulator {
  ...
  public assetBalance(assetId: bigint, ownership: Uint8Array): bigint {
    const res = this.contract.circuits.assetBalance(
      this.circuitContext, assetId, ownership
    );
    this.circuitContext = res.context;
    return res.result;
  }
  ...
  public setApproval(assetId: bigint, ownership: Uint8Array, target: Uint8Array, approved: boolean): Ledger {
    this.circuitContext = this.contract.impureCircuits.setApproval(
      this.circuitContext, assetId, ownership, this.buildEitherLeft(target), approved
    ).context;
    return ledger(this.circuitContext.transactionContext.state);
  }

  public isApproved(assetId: bigint, ownership: Uint8Array): boolean {
  ...
  public isApprovedOperator(operator: Uint8Array, assetId: bigint, ownership: Uint8Array): boolean {
    const res = this.contract.impureCircuits.isApprovedOperator(
      this.circuitContext, this.buildEitherLeft(operator), assetId, ownership
    );
    this.circuitContext = res.context;
    return res.result;
  }
  ...
  public createOffer(assetId: bigint, ownership: Uint8Array, shares: bigint, min: bigint, price: bigint, meta: string): Uint8Array {
    const res = this.contract.impureCircuits.createOffer(
      this.circuitContext, assetId, ownership, shares, min, price, meta
    );
    this.circuitContext = res.context;
    return res.result;
  }
  ...
  public genOfferId(ownership: Uint8Array, price: bigint): Uint8Array {
    return this.contract.circuits.genOfferId(
      this.circuitContext, ownership, price
    ).result;
  }
