...
describe("Ataulfo smart contract", () => {
  it("properly initializes ledger state and private state", () => {
    ...
    const hiddenContractOwner = simulator0.genHiddenOwner(encodeCoinPublicKey(pk));
  ...
  it("contract owner is the only one allowed to mint", () => {
    const pk = randomCoinPublicKeyHex();
    const simulator = new AtaulfoSimulator(randomBytes(32), pk);

    const assetId = 100n;
    const shares = 10n;
    const holdingId = simulator.mint(assetId, shares);
    expect(simulator.assetBalance(assetId, holdingId)).toEqual(shares);
    simulator.mint(assetId, shares);
    expect(simulator.assetBalance(assetId, holdingId)).toEqual(2n * shares);
    expect(() => simulator.assetBalance(10n, holdingId))
    ...
    expect(simulator.assetBalance(assetId, holdingId)).toBe(2n * shares);
  });

  it("set/unset approval", () => {
    const pk = randomCoinPublicKeyHex();
    const pk2 = randomCoinPublicKeyHex();
    const pwd = randomBytes(32);
    ...
    const holdingId = simulator.mint(assetId, shares);
    expect(simulator.isApproved(assetId, holdingId)).toBe(true);
    expect(simulator.isApprovedOperator(encodeCoinPublicKey(pk2), assetId, holdingId)).toBe(false);

    simulator.setApproval(assetId, holdingId, encodeCoinPublicKey(pk2), true);

    simulator.switchUser(pwd2, pk2);
    expect(simulator.isApproved(assetId, holdingId)).toBe(true);
    expect(simulator.isApprovedOperator(encodeCoinPublicKey(pk), assetId, holdingId)).toBe(true);

    simulator.setApproval(assetId, holdingId, encodeCoinPublicKey(pk2), false);
    expect(simulator.isApproved(assetId, holdingId)).toBe(false);
    expect(simulator.isApprovedOperator(encodeCoinPublicKey(pk), assetId, holdingId)).toBe(true);
  ...
  it("publishing an offer by asset owner", () => {
    ...
    const holdingId = simulator.mint(assetId, 10n);
    expect(() => simulator.createOffer(assetId, holdingId, 1n, 1n, 0n, meta))
      .toThrow("failed assert: Price must be greater than zero");
    expect(simulator.getLedger().offers.isEmpty()).toBe(true);
    const offerId = simulator.createOffer(assetId, holdingId, 1n, 1n, price, meta);
    expect(offerId).toEqual(simulator.genOfferId(simulator.getPrivateState().secretKey, holdingId, price));
  ...
  it("publish an offer allowed only to owner or approved", () => {
    ...
    // another party cannot create an offer since it has not balance of this asset
    const pk2 = randomCoinPublicKeyHex();
    const pwd2 = randomBytes(32);
    simulator.switchUser(pwd2, pk2);
    expect(() => simulator.createOffer(assetId, holdingId, 1n, 1n, price, meta))
  ...
  it("creating offers with same asset more than once", () => {
    ...
    const offerId1 = simulator.createOffer(assetId, holdingId, 1n, 1n, 1000n, meta);
    const offerId2 = simulator.createOffer(assetId, holdingId, 1n, 1n, 2000n, meta);
    ...
    const offerId3 = simulator.createOffer(assetId, holdingId, 1n, 1n, 2000n, newMeta);
    ...
    simulator.setApproval(assetId, holdingId, encodeCoinPublicKey(pk2), true);
    simulator.switchUser(pwd2, pk2);
    const offerId4 = simulator.createOffer(assetId, holdingId, 1n, 1n, 2000n, newMeta);
  ...
  it("asset owner can always cancel an offer published by operator", () => {
    ...
    // owner can always cancel even if it's not publisher
    simulator.switchUser(pwd, pk);
    const offer = simulator.cancelOffer(offerId);
    expect(offer.assetId).toEqual(assetId);
    expect(offer.price).toEqual(price);
  ...
  it("deposit funds", () => {
    ...
    const balanceOwner = simulator.genHiddenOwner(encodeCoinPublicKey(pk));
  ...
  it("fulfill an offer", () => {
    ...
    const assetId = 1200n;
    const shares = 12n;
    const shares_bought = 7n;
    const remaining_after_first_buy = shares - shares_bought;
    expect(remaining_after_first_buy).toBeGreaterThan(0n);
    const price = 100n;
    const meta = toHex(randomBytes(10));
    const holdingId = simulator.mint(assetId, shares);
    expect(simulator.myAssetBalance(assetId)).toBe(shares);

    const offerId = simulator.createOffer(assetId, holdingId, shares, 1n, price, meta);
    expect(simulator.getLedger().offers.member(offerId)).toBe(true);
    expect(() => simulator.fulfillOffer(offerId, 1n))
      .toThrow("failed assert: Offer cannot be fulfilled by the publisher");

    const pk2 = randomCoinPublicKeyHex();
    const pwd2 = randomBytes(32);
    simulator.switchUser(pwd2, pk2);
    expect(simulator.myAssetBalance(assetId)).toBe(0n);
    expect(() => simulator.fulfillOffer(offerId, shares_bought))
    ...
    const initialAmount = (price * shares_bought) + opsFee;
    ...
    expect(() => simulator.fulfillOffer(invalidOfferId, shares_bought))
      .toThrow("failed assert: Offer does not exist");
    ...
    // let's buy part of the offered shares
    let offer = simulator.fulfillOffer(offerId, shares_bought);
    let ledgerState = simulator.getLedger();
    expect(ledgerState.offers.isEmpty()).toBe(false);
    expect(ledgerState.offers.member(offerId)).toBe(true);
    expect(offer.shares).toEqual(shares);
    expect(offer.assetId).toEqual(assetId);
    expect(offer.price).toEqual(price);
    expect(offer.meta).toEqual(meta);
    expect(simulator.balance()).toBe(initialAmount + topUpAmount - (price * shares_bought) - 3n * opsFee);
    expect(simulator.getLedger().treasury.value).toEqual(initialAmount + topUpAmount - price);
    expect(simulator.myAssetBalance(assetId)).toBe(shares_bought);

    simulator.switchUser(pwd, pk);
    expect(simulator.myAssetBalance(assetId)).toBe(remaining_after_first_buy);

    // let's now buy all the reamining shares in the offer
    simulator.switchUser(pwd2, pk2);
    coinInfo = createCoinInfo(nativeToken(), (price * remaining_after_first_buy) + 2n * opsFee);
    simulator.depositFunds(coinInfo);
    offer = simulator.fulfillOffer(offerId, remaining_after_first_buy);
    ledgerState = simulator.getLedger();
    expect(ledgerState.offers.isEmpty()).toBe(true);
    expect(ledgerState.offers.member(offerId)).toBe(false);
    expect(offer.shares).toEqual(remaining_after_first_buy);
    expect(offer.assetId).toEqual(assetId);
    expect(offer.price).toEqual(price);
    expect(offer.meta).toEqual(meta);
    expect(simulator.balance()).toBe(0n);
    expect(simulator.myAssetBalance(assetId)).toBe(shares);
