  ...
  if (ledgerState === null) {
    ...
    if (ledgerState.offers.size() === 0n) {
      ...
      for (const [id, offer] of ledgerState.offers) {
        let publisher;
        if (offer.publisher.is_left) {
          publisher = toHex(offer.publisher.left.bytes);
        } else {
          publisher = toHex(offer.publisher.right.bytes);
        }

        logger.info(`- Id: ${toHex(id)}, publisher: ${publisher}, asset: ${offer.assetId}, price: ${offer.price}`);
      }
    }
...
const displayDerivedState = (ledgerState: AtaulfoDerivedState | undefined, logger: Logger) => {
  if (ledgerState === undefined) {
    logger.info(`No bulletin board state currently available`);
  } else {
    if (ledgerState.offers.size === 0) {
      logger.info(`No offers`);
    } else {
      logger.info(`Current offers:`);
      for (const [id, offer] of ledgerState.offers) {
        logger.info(`- Id: ${id}, asset: ${offer.assetId}, price: ${offer.price}`);
      }
    }
  }
};
    ...
    try {
      ...
      switch (choice) {
        ...
        case '2': {
          ...
          const offerId = await ataulfoApi.createOffer(BigInt(assetId.trim()), BigInt(price.trim()));
